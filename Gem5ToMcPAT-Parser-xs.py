...
def create_parser():
    ...
    parser.add_argument(
        '--output', '-o', type = str, default = "mcpat-in.xml",
        metavar = 'PATH',
...
def prepareTemplate(outputFile):
    numCores = 1
    print("-------------------------------------------------")
    if 'l2cache' in config["system"]:
        numL2 = numCores
    elif 'l2' in config["system"]:
        numL2 = 1
    else:
        numL2 = 0
    elemCounter = 0
...
def dumpMcpatOut(outFile):
    ...
    for param in rootElem.iter('param'):
        ...
        if 'config' in value:
            ...
            if len(allConfs) > 1:
                print("conf nums is greater than 1")
